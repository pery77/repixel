...
4. **RePalette** — reduce los colores a una paleta de [Lospec](https://lospec.com/palette-list): pega la URL (o el slug, o directamente códigos hex). O **abre una paleta del disco** con «Desde archivo…»: `.gpl` (GIMP, y lo que exportan Aseprite, Krita e Inkscape), `.pal` (tanto el de texto JASC como el binario de Windows), `.hex`, `.txt` (con el alfa delante, estilo Paint.NET), `.act` de Photoshop, y también **una imagen de muestras** —una tira o una cuadrícula de colores, que es como Lospec ofrece sus paletas en PNG—, de la que se sacan los colores distintos en orden de aparición. Lo que manda es el contenido del fichero, no la extensión. Los `.ase` de Adobe y los `.aseprite` no se leen: exporta a cualquiera de los de arriba. Distancia de color perceptual **OKLab** (o RGB ponderado), dithering **Floyd–Steinberg** o **Bayer 8×8** con intensidad regulable.
...
Los botones de acción son **iconos** con su explicación en el tooltip —cargar imágenes, carpeta, vaciar, tomar el color detectado, cargar paleta, abrir fichero, restablecer, zoom, ajustar, plegar y las dos descargas—; los desplegables se quedan en texto, porque «Promedio de área» frente a «Vecino más cercano» no es un dibujo sino una decisión.
