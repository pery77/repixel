...
2. **RePalette** — reducción de colores a una paleta de Lospec (URL/slug, con fallback a pegar códigos hex si falla CORS). Distancia perceptual OKLab (o RGB "redmean"), dithering Floyd–Steinberg o Bayer 8×8 con intensidad regulable.
3. **Máscaras RGB** — una textura para el shader con una máscara por canal: R metallic, G smoothness, B emisivo. **Rama aparte**: se calcula sobre el resultado ya palettizado y no modifica el diffuse. Cada canal tiene fuente (luminancia, máximo, saturación o un canal suelto), brillo, contraste, invertir y umbral, y se genera por plantilla desde `CANALES` para no repetir el HTML tres veces.
...
- ✅ Fase 1 — App funcional: carga de imágenes/carpetas (botones y arrastrar), pipeline ReSize → RePalette, vista previa original/resultado, descarga PNG y ZIP, config persistida en `localStorage` (`repixel_*`), tests en `pruebas.mjs` (110 comprobaciones).
